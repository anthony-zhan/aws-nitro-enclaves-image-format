...
impl PcrSigner for PrivateKeyPcrSigner {
    ...
    fn sign(&self, pcr_info: &PcrInfo) -> Result<Vec<u8>, String> {
        sign_pcr(pcr_info, &self.private_key)
    }
}
...
impl SignaturePcrSigner {
    pub fn new(cert_path: &str, sig_hex: &str) -> Result<Self, String> {
        let signature =
            hex::decode(sig_hex).map_err(|err| format!("Could not decode signature: {:?}", err))?;
        let mut certificate_file = File::open(cert_path)
            .map_err(|err| format!("Could not open the certificate file: {:?}", err))?;
        let mut signing_certificate = Vec::new();
        certificate_file
...
// Utility function to sign a PCR.
pub fn sign_pcr(pcr_info: &PcrInfo, private_key_pem: &[u8]) -> Result<Vec<u8>, String> {
    let payload =
        to_vec(pcr_info).map_err(|err| format!("Could not serialize PCR info: {:?}", err))?;
    ...
    let signature = CoseSign1::new::<Openssl>(&payload, &HeaderMap::new(), private_key.as_ref())
        .unwrap()
        .as_bytes(false)
        .unwrap();
